...
#include "briabridge.moc"

BriaBridge::BriaBridge(BriaGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void BriaBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void BriaBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void BriaBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void BriaBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void BriaBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void BriaBridge::populateOptions()
{
...
// Transactions
bool BriaBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void BriaBridge::clearRecipients()
{
    recipients.clear();
}

bool BriaBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 BRIA, 1 Bria
        switch(rcp.txnTypeInd)
        {
            case TXT_BRIA_TO_BRIA:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BRIA, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_BRIA_TO_ANON:
                formatted.append(tr("<b>%1</b> to SHADOW %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BRIA, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> SHADOW, ring size %2 to SHADOW %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BRIA, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_BRIA:
                formatted.append(tr("<b>%1</b> SHADOW, ring size %2 to BRIA %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BRIA, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BRIA, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsBriaBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your bria balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BRIA, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void BriaBridge::openCoinControl()
{
...
void BriaBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void BriaBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap BriaBridge::listAnonOutputs()
{
...
void BriaBridge::populateTransactionTable()
{
...
void BriaBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void BriaBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString BriaBridge::transactionDetails(QString txid)
{
...
// Addresses
void BriaBridge::populateAddressTable()
{
...
void BriaBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void BriaBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString BriaBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString BriaBridge::lastAddressError()
{
    QString sError;
...
QString BriaBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void BriaBridge::updateAddressLabel(QString address, QString label)
{
...
bool BriaBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool BriaBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void BriaBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void BriaBridge::appendMessage(int row)
{
...
void BriaBridge::populateMessageTable()
{
...
void BriaBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool BriaBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool BriaBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString BriaBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString BriaBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool BriaBridge::setPubKey(QString address, QString pubkey)
{
...
bool BriaBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString BriaBridge::createGroupChat(QString label)
{
...
QString BriaBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList BriaBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void BriaBridge::connectSignals()
{
...
QString BriaBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap BriaBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap BriaBridge::listLatestBlocks()
{
...
QVariantMap BriaBridge::findBlock(QString searchID)
{
...
QVariantMap BriaBridge::blockDetails(QString blkHash)
{
...
QVariantMap BriaBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap BriaBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap BriaBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap BriaBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap BriaBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap BriaBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap BriaBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap BriaBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap BriaBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Briacoin prefix.");
            return result;
        }
...
QVariantMap BriaBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap BriaBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap BriaBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
