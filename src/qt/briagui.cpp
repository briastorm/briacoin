// Copyright (c) 2014 The BriaCoin developers
...
BriaGUI::BriaGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new BriaBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("Bria") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/bria"));
    setWindowIcon(QIcon(":icons/bria"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/bria/index.html");
#else
    QFile html("/opt/bria/index.html");
#endif
...
void BriaGUI::pageLoaded(bool ok)
{
...
void BriaGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void BriaGUI::urlClicked(const QUrl & link)
{
...
void BriaGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bria"), tr("&About BriaCoin"), this);
    aboutAction->setToolTip(tr("Show information about BriaCoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for BriaCoin"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/bria"), tr("&Show / Hide"), this);
...
void BriaGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("Bria") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/bria_testnet"));
            setWindowIcon(QIcon(":icons/bria_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/bria_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Bria client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/bria_testnet"));
...
void BriaGUI::setWalletModel(WalletModel *walletModel)
{
...
void BriaGUI::setMessageModel(MessageModel *messageModel)
{
...
void BriaGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Bria client"));
    trayIcon->setIcon(QIcon(":/icons/bria"));
...
#ifndef Q_OS_MAC
void BriaGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BriaGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to BriaCoin network", "", count));
}

void BriaGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void BriaGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void BriaGUI::changeEvent(QEvent *e)
{
...
void BriaGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void BriaGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::BRIA, nFeeRequired));
...
void BriaGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void BriaGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void BriaGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void BriaGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BriaGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void BriaGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid BriaCoin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void BriaGUI::handleURI(QString strURI)
{
...
void BriaGUI::setEncryptionStatus(int status)
{
...
void BriaGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void BriaGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void BriaGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BriaGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void BriaGUI::updateStakingIcon()
{
