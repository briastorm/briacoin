...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return BriaGUI::tr("Received with");
    case RecvFromOther:
        return BriaGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return BriaGUI::tr("Sent to");
    case SendToSelf:
        return BriaGUI::tr("Payment to yourself");
    case Generated:
        return BriaGUI::tr("Mined");
    case RecvBria:
        return BriaGUI::tr("Received bria");
    case SendBria:
        return BriaGUI::tr("Sent bria");
    case Other:
        return BriaGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvBria:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendBria:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredBRIA, nCredBria;
    wtx.GetCredit(nCredBRIA, nCredBria, true);
    int64_t nCredit = nCredBRIA + nCredBria;
    ...
    {
        if (nNet > 0 && nCredBria > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvBria, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendBria, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvBria;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv bria
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendBria;
                    sub.address = CBitcoinAddress(ckidD).ToString();
