...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to briacoind / RPC client
            std::string strUsage = _("BriaCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  briacoind [options]                     " + "\n" +
                  "  briacoind [options] <command> [params]  " + _("Send command to -server or briacoind") + "\n" +
                  "  briacoind [options] help                " + _("List commands") + "\n" +
                  "  briacoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "briacoin:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect briacoind signal handlers
    noui_connect();
