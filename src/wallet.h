...
public:
    ...
    int64_t GetBriaDebit(const CTxIn& txin) const;
    int64_t GetBriaCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nBRIA, int64_t& nBria) const
    {
        nBRIA = 0;
        nBria = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nBria += GetBriaCredit(txout);
            } else
                nBRIA += GetCredit(txout);
            if (!MoneyRange(nBRIA)
                || !MoneyRange(nBria))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableBriaCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableBriaCreditCached;
    
    mutable int64_t nCredBRIACached;
    mutable int64_t nCredBriaCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableBriaCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredBRIA, int64_t& nCredBria, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredBRIA = nCredBria = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredBRIACached = 0;
            nCredBriaCached = 0;
            pwallet->GetCredit(*this, nCredBRIACached, nCredBriaCached);
            fCreditSplitCached = true;
        };
        
        nCredBRIA = nCredBRIACached;
        nCredBria = nCredBriaCached;
        return true;
    }
    ...
    int64_t GetAvailableBriaCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableBriaCreditCached)
            return nAvailableBriaCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableBriaCredit() : value out of range");
            };
        };
        
        nAvailableBriaCreditCached = nCredit;
        fAvailableBriaCreditCached = true;
        return nCredit;
    };
